...
class TransLayerShare(nn.Module):
    def __init__(self, dim, kernel_size):
        super(TransLayerShare, self).__init__()
...
class TransLayerNotShare(nn.Module):
    def __init__(self, dim, kernel_size):
        super(TransLayerNotShare, self).__init__()
        ...
        self.conv_q = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, groups=4, bias=False),
            nn.BatchNorm2d(dim),
            nn.ReLU(inplace=True) ###########################################
        )
        ...
        self.conv1x1 = nn.Sequential(
            #nn.Conv2d(dim, dim, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, groups=4, bias=False),
            nn.Conv2d(dim, dim, kernel_size=self.kernel_size, stride=1, padding=self.kernel_size//2, groups=2, bias=False), ###########
            nn.BatchNorm2d(dim)
        )
...
class TransLayer1x1(nn.Module):
    def __init__(self, dim, kernel_size):
        super(TransLayer1x1, self).__init__()
        ...
        self.key_embed = nn.Sequential(
            #nn.Conv2d(dim, dim, self.kernel_size, stride=1, padding=self.kernel_size//2, groups=4, bias=False),
            nn.Conv2d(dim, dim, kernel_size=1, stride=1, padding=0, groups=4, bias=False),
            nn.BatchNorm2d(dim),
            nn.ReLU(inplace=True)
        )
...
class BottleneckShare(Bottleneck):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d,
                 attn_layer=None, aa_layer=None, drop_block=None, drop_path=None):
        super(BottleneckShare, self).__init__(inplanes, planes, stride, downsample, cardinality, base_width,
                 reduce_first, dilation, first_dilation, act_layer, norm_layer,
                 attn_layer, aa_layer, drop_block, drop_path)
        width = int(math.floor(planes * (base_width / 64)) * cardinality)
        self.conv2 = TransLayerShare(width, kernel_size=3)

class BottleneckNotShare(Bottleneck):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d,
                 attn_layer=None, aa_layer=None, drop_block=None, drop_path=None):
        super(BottleneckNotShare, self).__init__(inplanes, planes, stride, downsample, cardinality, base_width,
                 reduce_first, dilation, first_dilation, act_layer, norm_layer,
                 attn_layer, aa_layer, drop_block, drop_path)
        width = int(math.floor(planes * (base_width / 64)) * cardinality)
        self.conv2 = TransLayerNotShare(width, kernel_size=3)

class Bottleneckkey1x1(Bottleneck):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d,
                 attn_layer=None, aa_layer=None, drop_block=None, drop_path=None):
        super(Bottleneckkey1x1, self).__init__(inplanes, planes, stride, downsample, cardinality, base_width,
                 reduce_first, dilation, first_dilation, act_layer, norm_layer,
                 attn_layer, aa_layer, drop_block, drop_path)
        width = int(math.floor(planes * (base_width / 64)) * cardinality)
        self.conv2 = TransLayer1x1(width, kernel_size=3)

class Bottleneck5x5(Bottleneck):
    expansion = 4

    def __init__(self, inplanes, planes, stride=1, downsample=None, cardinality=1, base_width=64,
                 reduce_first=1, dilation=1, first_dilation=None, act_layer=nn.ReLU, norm_layer=nn.BatchNorm2d,
                 attn_layer=None, aa_layer=None, drop_block=None, drop_path=None):
        super(Bottleneck5x5, self).__init__(inplanes, planes, stride, downsample, cardinality, base_width,
                 reduce_first, dilation, first_dilation, act_layer, norm_layer,
                 attn_layer, aa_layer, drop_block, drop_path)
        width = int(math.floor(planes * (base_width / 64)) * cardinality)
        self.conv2 = TransLayer(width, kernel_size=5)
...
@register_model
def cotnet50_share(pretrained=False, **kwargs):
    model_args = dict(block=BottleneckShare, layers=[3, 4, 6, 3],  **kwargs)
    return _create_trans_resnet('trans_basic', pretrained, **model_args)

@register_model
def cotnet50_notshare(pretrained=False, **kwargs):
    model_args = dict(block=BottleneckNotShare, layers=[3, 4, 6, 3],  **kwargs)
    return _create_trans_resnet('trans_basic', pretrained, **model_args)

@register_model
def cotnet50_1x1(pretrained=False, **kwargs):
    model_args = dict(block=Bottleneckkey1x1, layers=[3, 4, 6, 3],  **kwargs)
    return _create_trans_resnet('trans_basic', pretrained, **model_args)

@register_model
def cotnet50_5x5(pretrained=False, **kwargs):
    model_args = dict(block=Bottleneck5x5, layers=[3, 4, 6, 3],  **kwargs)
    return _create_trans_resnet('trans_basic', pretrained, **model_args)